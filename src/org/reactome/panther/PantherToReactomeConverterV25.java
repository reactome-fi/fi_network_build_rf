 ...
 * between some of the celldesigner-versions, this importer will only handle the versions 2.5 and 4.0. For the
 * import of versions 2.2 and 2.3, please go to <code>PantherToReactomeConverterV25</code>.<br><br>
 * 
 * This class is derived from the 2.2/2.3 importer and is modified to work with 2.5/4.0 with panther pathways.
 * It is possible that there is code that is not used anymore, I only removed locally unused variables/methods
 * after the change, but I didn't look through all the code for unused stuff (just made sure it works).<br>
 * 
 * @author guanming
 * @author andreash
 */
@SuppressWarnings("unchecked")
public class PantherToReactomeConverterV25 implements Converter {
	...
	public PantherToReactomeConverterV25() {
	...
	public GKInstance convertBeforePost(String fileName) throws JDOMException, IOException, Exception {
		...
		Element includedSpeciesElm = annotationElm.getChild(PantherConstants.LIST_OF_INCLUDED_SPECIES_ELM_NAME,
				PantherConstants.CELL_DESIGNER_NS);
		processSpeciesElement(speciesListElm, includedSpeciesElm);
	...
	/**
	 * Some specieses have active states. A new PhysicalEntity should be created for an active state.
	 * @param modelElm
	 * @throws Exception
	 */
	private void handleActiveStates(Element modelElm) throws Exception {
		...
		Map<String, String> complexToCompartmentMap = extractComplexCompartmentMapping(modelElm);
		List elements;
		{  // combining simple and complex species alias into one list
			String xPath = ".//celldesigner:speciesAlias";
			XPath myPath = XPathHelper.getXPathObject(xPath, modelElm);
			elements = myPath.selectNodes(modelElm);
			xPath = ".//celldesigner:complexSpeciesAlias";
			myPath = XPathHelper.getXPathObject(xPath, modelElm);
			List complexAliasElements = myPath.selectNodes(modelElm);
			if (elements == null)
				elements = complexAliasElements;
			else if (complexAliasElements != null)
				elements.addAll(complexAliasElements);
		}
...
//			if (elm.getAttributeValue(PantherConstants.COMPLEX_SPECIES_ALIAS_ATT_NAME)!=null)
//				continue;
			String compartmentAlias = elm.getAttributeValue(PantherConstants.COMPARTMENT_ALIAS_ATT_NAME);
			if (compartmentAlias == null) { // look in the alternative place for the compartment alias
				compartmentAlias = complexToCompartmentMap.get(
						elm.getAttributeValue(PantherConstants.COMPLEX_SPECIES_ALIAS_ATT_NAME));
			}
	...
	/**
	 * As some species entries only have the complex id they belong to,
	 * this map is used to map complex ids to according compartment ids
	 * @param modelElm Element
	 * @return Map&lt;ComplexId, CompartmentId>
	 * @throws JaxenException
	 */
	private Map<String, String> extractComplexCompartmentMapping(Element modelElm) throws JaxenException{
		Map<String, String> map = new HashMap<String, String>();
		String xPath = ".//celldesigner:complexSpeciesAlias";
		XPath myPath = XPathHelper.getXPathObject(xPath, modelElm);
		...
		for (Iterator it = elements.iterator(); it.hasNext();) {
			Element elm = (Element) it.next();
			String alias = elm.getAttributeValue(PantherConstants.COMPARTMENT_ALIAS_ATT_NAME);
			String id = elm.getAttributeValue(PantherConstants.ID_ATT_NAME);
			map.put(id, alias);
		}
		return map;
	}
	...
	private void processReactionsElement(Element reactionsElm) throws Exception {
		...
		for (Iterator it = reactionElmList.iterator(); it.hasNext();) {
			...
			if (isInhibition(rxtElm))
				...
				idToReactionMap.put(rxtElm.getAttributeValue(PantherConstants.ID_ATT_NAME), createReaction(
				      rxtElm, reactionMap));
...
//				nr.setAttributeValue(ReactomeJavaConstants.regulationType, regulationType);
			...
			// Sometimes the inhibited species doesn't participate any reactions. See Akt
			// in Angiogensis.xml for an example. Wrap both input and output in a reaction.
			...
			idToReactionMap.put(rxtElm.getAttributeValue(PantherConstants.ID_ATT_NAME), reaction);
		}
	}
	...
	private void extractCoordinates(Element annotationElm) throws JaxenException {
		...
		Map<String, String> coordSet;

		// getting the coordinates for the entire pathway via "modelDisplay" coordinates will
		// be saved like other coordinates, but under the key of ReactomeConstants.PathwayDiagram
		// It's wrapped in a namespace to keep the number of declared variables low afterwards
		{
			...
			if (list != null && list.size() > 0) {
				...
				coordSet.put(PantherConstants.X_ATT_NAME, "0");
				coordSet.put(PantherConstants.Y_ATT_NAME, "0");
		...
		// continuing with rest of the coordinates
		{
			...
			List complexAliasElements = myPath.selectNodes(annotationElm);
			if (list == null)
				list = complexAliasElements;
			else if (complexAliasElements != null)
				list.addAll(complexAliasElements);
		}
		if (list != null && list.size() > 0) {
			String alias;
			String species;
	...
	private void extractAliasMatching(Element reactionsElm) throws JaxenException {
		...
		if (list != null && list.size() > 0) {
			for (Iterator it = list.iterator(); it.hasNext();) {
				...
				String reactionID = reactionElm.getAttributeValue(PantherConstants.ID_ATT_NAME);
				Element annElm = reactionElm.getChild(PantherConstants.ANNOTATION_ELM_NAME,
				      PantherConstants.SBML_NS);
				
				String baseInputPath = ".//celldesigner:baseReactant[@alias]";
				String baseOutputPath = ".//celldesigner:baseProduct[@alias]";
				String inputAliasPath = ".//celldesigner:reactantLink[@alias]";
				String outputAliasPath = ".//celldesigner:productLink[@alias]";
				String catalystPath = ".//celldesigner:modification[@aliases]";

				List<String> baseInput = getAttributes(annElm, baseInputPath);
				List<String> baseOutput = getAttributes(annElm, baseOutputPath);
				List<String> input = getAttributes(annElm, inputAliasPath);
				List<String> output = getAttributes(annElm, outputAliasPath);
				List<String> catalyst = getAttributes(annElm, catalystPath);
	...
	private List<String> getAttributes(Element annotationElm, String xPath) {
		XPath myPath = null;
		List list = null;
		try {
			myPath = XPathHelper.getXPathObject(xPath, annotationElm);
		...
		if (list != null && list.size() != 0) {
			for (Iterator it = list.iterator(); it.hasNext();) {
				Element elm = (Element) it.next();
				String alias = getAttributeFromXPath(xPath);
				out.add(elm.getAttributeValue(alias));
			}
		}
	...
	/**
	 * returns the attribute that is looked for in an xPath String as
	 * somehow the XPath Java APIs don't seem to be able to handle that
	 * @param xPath String
	 * @return The Attribute name
	 */
	private String getAttributeFromXPath(String xPath) {
	...
	/**
	 * Processes secpies, normal and included 
	 * @param speciesListElm
	 * @param listOfIncludedSpeciesElm
	 * @throws Exception
	 */
	private void processSpeciesElement(Element speciesListElm, Element listOfIncludedSpeciesElm) throws Exception {		
		List<Element> elements = new ArrayList<Element>(); 
		{
			List elmList = speciesListElm.getChildren(PantherConstants.SPECIES_ELM_NAME, PantherConstants.SBML_NS);
			List inclElmList;
			if (listOfIncludedSpeciesElm==null)
				inclElmList=null;
			else
				inclElmList = listOfIncludedSpeciesElm.getChildren(
						PantherConstants.SPECIES_ELM_NAME, PantherConstants.CELL_DESIGNER_NS);
			if (elmList!=null)
				elements.addAll(elmList);
			if (inclElmList != null)
				elements.addAll(inclElmList);
		}
		...
		for (Element complexElm : complexElms) {
			processComplexComponents(complexElm, listOfIncludedSpeciesElm);
		}
		...
		for (GKInstance instance : complexList) {
			...
			String compartment = instance.getDisplayName().substring(name.length());
			String complexName = "";
			...
			instance.setAttributeValue(ReactomeJavaConstants._displayName, complexName + compartment);
		}
	}

	private void processComplexComponents(Element complexElm, Element listOfIncludedSpeciesElm) throws Exception {
		...
		String xPath = ".//celldesigner:" + PantherConstants.ANNOTATION_ELM_NAME;
		XPath myPath = XPathHelper.getXPathObject(xPath, listOfIncludedSpeciesElm);
		List entries = myPath.selectNodes(listOfIncludedSpeciesElm);
		if (entries == null || entries.size() == 0)
			return;
		for (Iterator it = entries.iterator(); it.hasNext();) {
			Element entryElm = (Element) it.next();
			String speciesName = entryElm.getChildText(PantherConstants.COMPLEX_SPECIES_ELM_NAME,
					PantherConstants.CELL_DESIGNER_NS);
			if (speciesName.equals(id)) {
				Element speciesElm = entryElm.getChild(PantherConstants.SPECIES_IDENTITY_ELM_NAME,
						PantherConstants.CELL_DESIGNER_NS);
				handleComplexComp(speciesElm, complex);
			}
		}
	...
	private GKInstance createEntity(Element elm) throws Exception {
		...
		if (annotationElm == null) 
			annotationElm = elm.getChild(PantherConstants.ANNOTATION_ELM_NAME, PantherConstants.CELL_DESIGNER_NS);
